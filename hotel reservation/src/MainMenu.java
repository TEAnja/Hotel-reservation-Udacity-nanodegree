    ...
    private static void findAndReserveARoom() {
        ...
        if (customer != null){
            ...
            if(HotelResource.findARoom(checkIn, checkOut) == null || HotelResource.findARoom(checkIn, checkOut).isEmpty()) {
                ...
                Calendar checkInCal = Calendar.getInstance(); //CHANGING THE CHECK IN DATE
                checkInCal.setTime(checkIn);
                checkInCal.add(Calendar.DAY_OF_MONTH, 7);
                Date checkIn1 = checkInCal.getTime();

                Calendar checkOutCal = Calendar.getInstance(); // CHANGING THE CHECK OUT DATE
                checkOutCal.setTime(checkOut);
                checkOutCal.add(Calendar.DAY_OF_MONTH, 7);
                Date checkOut1 = checkOutCal.getTime();
                Collection<IRoom> recommendedRooms = HotelResource.findARoom(checkIn1, checkOut1);
                System.out.println("We recommend these rooms for alternative dates from " + checkIn1 + " to " + checkOut1 + ":");
                for(IRoom recommendedRoom : recommendedRooms){
                    System.out.println("Room number " + recommendedRoom.getRoomNumber() + ", room type: " + recommendedRoom.getRoomType() + "; price: " + recommendedRoom.getRoomPrice());
                }
                //-----
                ...
                try{
                    while(true){
                        ...
                        if(HotelResource.findARoom(checkIn1, checkOut1).contains(HotelResource.getRoom(roomNumber))){
                            IRoom room = HotelResource.getRoom(roomNumber);

                            HotelResource.bookARoom(email, room, checkIn1, checkOut1);
                            System.out.println("Your reservetion from " + checkIn1 +
                                    " to " + checkOut1 + ", room '" +
